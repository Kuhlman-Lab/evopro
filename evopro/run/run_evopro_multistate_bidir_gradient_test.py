...
from evopro.genetic_alg.geneticalg_helpers import read_starting_seqs, create_new_seqs, create_new_seqs_mpnn, mutate_by_protein_mpnn
...
from evopro.score_funcs.score_multistate_henry import BIDIR_MATRIX, ALPHABET
    ...
    while curr_iter <= num_iter:
        ...
        #using protein mpnn to refill pool when specified
            ...
            pool = create_new_seqs_mpnn_henry_bidir(pool, 
                                        ...
                                        bias_by_res_dict=bias_by_res_dict, bidir=bidir)
            mpnn_counter+=1
        ...
        for seqs, results, dsobj in zip(seqs_packed, results_packed, scoring_pool):
            if path_to_starting is not None:   
                scores.append(score_func(results, path_to_starting, curr_iter, num_iter, dsobj))
            else:
                scores.append(score_func(results, dsobj, curr_iter, num_iter, contacts=contacts))
...
if __name__ == "__main__":
    ...
    if args.path_to_starting:
        try:
            path_to_starting = args.path_to_starting
        except:
            raise ValueError("Please provide path to pdb.")
    else:
        path_to_starting=None    
