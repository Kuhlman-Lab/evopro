...
def run_genetic_alg_gpus(run_dir, af2_flags_file, score_func, startingseqs, poolsize = 40, num_iter = 50, n_workers=2, write_raw_plddts=False, write_pair_confs=False, rmsd_func=None ,write_pdbs=False, mpnn_iters=None, len_binder=0, use_of = False, crossover_percent = 0.2, vary_length = 0, mut_percents=None, stabilize_monomer=None):
        ...
        if stabilize_monomer:
            for pdb, k in zip(pdbs, range(len(pdbs))):
                if k==0:
                    ...
                    with open(run_dir + "seq_" + str(j) + "_final_model_1_binderonly_chain" + stabilize_monomer[k-1] + ".pdb", "w") as pdbf:
                        pdbf.write(str(pdb))
        else:
            with open(run_dir + "seq_" + str(j) + "_final_model_1_complex.pdb", "w") as pdbf:
                        pdbf.write(str(pdbs[0]))
