...
def score_monomer(results, diff_backbone):
    from alphafold.common import protein
    ...
    chains0, residues0, resindices0 = get_coordinates_pdb(pdbs[0])
    reslist0 = [x for x in residues0.keys()]
    confscore = score_plddt_confidence(results[0], reslist0, resindices0)
    rmsd_todiff_score = score_rmsd_to_starting(pdbs[0], diff_backbone)
    score = -confscore/10 + rmsd_todiff_score

    return score, (confscore, rmsd_todiff_score), pdbs, results
