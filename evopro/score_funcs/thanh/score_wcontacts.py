...
def score(results, dsobj, starting_pdb=None, contacts=None):
    ...
    reslist_A = [x for x in residues.keys() if int(x[1:len(x)]) in range(1,16)]
    reslist_B = [x for x in residues.keys() if int(x[1:len(x)]) in range(22,46)]
    reslist_C = [x for x in residues.keys() if int(x[1:len(x)]) in range(65,76)]
    ...
    plddt_C = score_plddt_confidence(results, reslist_C, resindices, dsobj=dsobj, first_only=False)
    plddt_weight = 2*plddt_A + plddt_B + 2*plddt_C
    # other terms
    ...
    score = -confscore + rmsd_score + contact_score - plddt_weight
    #print(score)
...
def score_contact(results, pdb, dsobj, orient=None):
    reslist1 = ['A4','A70']
    reslist2 = ['A13','A41','A63'] #[x for x in residues.keys() if x.startswith("B")]
    # set distance cut-off as 8A for now
...
def score_binder_rmsd_to_starting(pdb, contacts, starting_pdb=None, dsobj=None):
    ...
    # change rmsd_cutoff from 4A to 2A
    rmsd_cutoff = 2.0
...
### old score function used for trial 1
