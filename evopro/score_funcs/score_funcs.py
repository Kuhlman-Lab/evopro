...
def score_contacts(pdbfile, reslist1, reslist2, fil = False, orient=None, dist=4, orient_dist=20, of=False, score_cap=36):
    if fil:
...
def get_rmsd(reslist1, pdb1, reslist2, pdb2, ca_only=False, translate=True):
    ...
    A = A.astype(float)
    B = B.astype(float)
    from folddesign.score_funcs.calculate_rmsd import kabsch_rmsd
...
if __name__ == "__main__":
    ...
    chains, residues, resindices = get_coordinates_pdb(pdb2, fil = True)
