...
def run_protein_mpnn(pdb_dir, jsonstring, mpnn_temp, mpnn_version="v_48_020", bidir=False, dump_probs=False):
    sys.path.append('/proj/kuhl_lab/proteinmpnn/run/')
    from run_protein_mpnn import run_protein_mpnn_func
    results = run_protein_mpnn_func(pdb_dir, jsonstring, sampling_temp=mpnn_temp, model_name=mpnn_version, bidir=bidir, dump_probs=dump_probs)

    return results

def run_mpnn_on_diff_backbone(pdb_backbone, jsonfile, output_dir, mpnn_temp="0.1", mpnn_version="s_48_020", dump_probs=False):
    with open(jsonfile, 'r') as f:
        jsondata = f.read()

    new_seq = run_protein_mpnn(pdb_backbone, jsondata, mpnn_temp, mpnn_version=mpnn_version, bidir=False, dump_probs=dump_probs)
...
def getFlagParser() -> FileArgumentParser:
    ...
    parser.add_argument('--dump_probs', action='store_true', help='If enabled, a file (probs.pkl) containing the aa probabilities at each position is saved.')
    return parser
            ...
            if not os.path.isdir(mpnn_dir):
                for i in range(0,num_seqs_mpnn):
                    ...
                    if not os.path.isdir(mpnn_run_dir):
                        ...
                        run_mpnn_on_diff_backbone(mpnn_run_dir, jsonfile, mpnn_run_dir, mpnn_temp=args.mpnn_temp, mpnn_version=args.mpnn_version, dump_probs=args.dump_probs)
