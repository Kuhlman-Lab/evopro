...
from evopro.utils.gap_or_ungap_chains import gap_or_ungap_chains

sys.path.append('/proj/kuhl_lab/alphafold/run')
from run_af2 import af2_init

sys.path.append('/proj/kuhl_lab/PIPPack/')
from inference import run_pippack
...
def run_af2_on_mpnn_seqs(diff_backbone, mpnn_seqs_list, score_func, af2_flags_file, output_dir,  n_workers=1):
    ...
    for seq,i in zip(sorted_seqs_and_scores, range(len(sorted_seqs_and_scores))):
        ...
        with open(os.path.join(output_dir, "seq_" + str(i) + "_af2/" + "seq_" + str(i) + "_model_1.pdb"), "w") as pdbf:
            pdbf.write(str(pdb))
        compressed_pickle(os.path.join(output_dir, "seq_" + str(i) + "_af2/" + "seq_" + str(i) + "_result"), r)
...
def run_af2_on_mpnn_seqs_withmonomers(dist, diff_backbone, work_list, score_func, num_preds, output_dir):
    ...
    for seq,i in zip(sorted_seqs_and_scores, range(len(sorted_seqs_and_scores))):
        ...
        for pdb, name in zip(pdbs, names):
            with open(os.path.join(output_dir, "seq_" + str(i) + "_af2/" + "seq_" + str(i) + "_" + name + ".pdb"), "w") as pdbf:
                pdbf.write(str(pdb))
        for r in rs:
            compressed_pickle(os.path.join(output_dir, "seq_" + str(i) + "_af2/" + "seq_" + str(i) + "_" + name + "_result"), r)
...
if __name__ == "__main__":
    ...
    if not os.path.isdir(output_dir):
        ...
        print("Outputs directory already exists. Deleting and creating new directory.")
    ...
    if args.max_chains_length:
        ...
        raise ValueError("Please provide max_chains_length argument. Specify max length of each chain, separated by commas.")
    ...
    for pdb in pdbs:
        ...
        for element, i in zip(work_list, range(len(work_list))):
            af2_dir = os.path.join(mpnn_dir, "seq_" + str(i) + "_af2/")
            os.makedirs(af2_dir)
            element.append(partial(change_dir, af2_dir))
            print([element])
            pdb_strings = run_pippack(pdb_backbone, sequences=[element[0]], write_pdbs=True)
            print(len(pdb_strings))
            with open(os.path.join(af2_dir, "before_gapping.pdb"), "w") as f:
                f.write(pdb_strings[0])
            gapped_pdb = gap_or_ungap_chains(pdb_strings[0], mode="gap")
        
            if templates_dir:
                shutil.copytree(templates_dir, os.path.join(af2_dir, "templates/"))
            
            if not os.path.exists(os.path.join(af2_dir, "templates/")):
                os.makedirs(os.path.join(af2_dir, "templates/"))
            
            with open(os.path.join(af2_dir, "templates/temp.pdb"), "w") as f:
                f.write(gapped_pdb)
                
            work_list[i][0] = ":".join(element[0])
    ...
    if len(all_results)>0:
        ...
        with open(os.path.join(output_dir, "all_seqs_and_scores.csv"), "w") as opf:
            for seq,i in zip(sorted_results, range(len(sorted_results))):
                opf.write(str(seq[0]) + "\t" + str(seq[1]) + "\t" + str(seq[2]) + "\n")