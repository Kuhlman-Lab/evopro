...
def score_overall(results, dsobj, contacts=None, distance_cutoffs=None, rmsd_pdb=None):
    ...
    for result in results:
        ...
        if len(chains)>1:
            ...
            if rmsd_pdb:
                score.append(score_binder_rmsd_to_starting(pdb, rmsd_pdb, dsobj=dsobj))
...
def score_pip2_contacts_enth_e10(results, dsobj, contacts=None, distance_cutoffs=None, starting_pdb=None):
    #To score the contacts between the two domains of the binder use the contacts flag and the bonus contacts flag
    from alphafold.common import protein
    print("Number of predictions being scored:", len(results))
    ...
    for result in results:
        ...
        if len(chains)>1:
            ...
            res5 = coords[14 + 244]
            res6 = coords[69 + 244]
            orient2 = coords[43 + 244]

            angle, mix = calculate_plane_from_points(res1, res2, res3, res4, res5, res6, orient1, orient2)

            angle_constant = 10.0
            angle_cutoff1 = 30.0
            angle_cutoff2 = 45.0
            #exponential potential function for angle
            if abs(angle) > angle_cutoff:
                angle_potential = angle_constant*math.pow(angle - angle_cutoff, 2)
            else:
                angle_potential = angle/5
            score.append((angle_potential, angle_potential))
        else:
            score.append(score_binder_monomer(result, dsobj))
            if starting_pdb:
...
def score_pip2_contacts_enth_e106(results, dsobj, contacts=None, distance_cutoffs=None, starting_pdb=None):
    #To score the contacts between the two domains of the binder use the contacts flag and the bonus contacts flag
    from alphafold.common import protein
    print("Number of predictions being scored:", len(results))
...
def score_pip2_contacts_enth_e239(results, dsobj, contacts=None, distance_cutoffs=None, starting_pdb=None):
    #To score the contacts between the two domains of the binder use the contacts flag and the bonus contacts flag
    from alphafold.common import protein
    print("Number of predictions being scored:", len(results))
...
def score_pip2_contacts_ph_p106(results, dsobj, contacts=None, distance_cutoffs=None, starting_pdb=None):
    #To score the contacts between the two domains of the binder use the contacts flag and the bonus contacts flag
    from alphafold.common import protein
    print("Number of predictions being scored:", len(results))
...
def score_pip2_contacts_ph_p397(results, dsobj, contacts=None, distance_cutoffs=None, starting_pdb=None):
    #To score the contacts between the two domains of the binder use the contacts flag and the bonus contacts flag
    from alphafold.common import protein
    print("Number of predictions being scored:", len(results))
...
def score_pip2_contacts_ph(results, dsobj, contacts=None, distance_cutoffs=None, starting_pdb=None):
    #To score the contacts between the two domains of the binder use the contacts flag and the bonus contacts flag
    from alphafold.common import protein
    print("Number of predictions being scored:", len(results))
    ...
    for result in results:
        ...
        angle, mix = calculate_plane_from_points(res1, res2, res3, res4, res5, res6)
...
def score_binder_within(results, dsobj, contacts, distance_cutoffs):
    from alphafold.common import protein
    pdb = protein.to_pdb(results['unrelaxed_protein'])
    chains, residues, resindices = get_coordinates_pdb(pdb)
    ...
    score = -contactscore
    print(score, (score, len(contact_list), contactscore, pae_per_contact))
    return score, (score, len(contact_list), contactscore, pae_per_contact), contacts, pdb, results
