...
def score_binder_complex_rmsd(results, dsobj, contacts, distance_cutoffs):
    start = time.time()
    pdb = results['pdb']
    chains, residues, resindices = get_coordinates_pdb(pdb)
    print(f"contacts === {contacts}")
    ...
    if num_contacts > 0:
        pae_per_contact = (70.0-(70.0*contactscore)/num_contacts)/2
        rmsdscore = 0    
    rmsd_pdb = "/nas/longleaf/home/zyyao/project/96.evopro_rf2_rmsd/diff_29_hspt03_Complex_base_NoROG_1.pdb"
    rmsdscore = score_rmsd_to_starting(pdb, rmsd_pdb)
    
    score = -contactscore + penalty + bonus + rmsdscore
    print("Time to score:", time.time()-start)
    print(score, (score, len(contact_list), contactscore, pae_per_contact, bonus, penalty, rmsdscore))
    return score, (score, len(contact_list), contactscore, pae_per_contact, bonus, penalty, rmsdscore), contacts, pdb, results 
