...
def run_mpnn_on_diff_backbone_inputmultimer(pdb_backbone, jsonfile, output_dir, mpnn_temp="0.1", mpnn_version="s_48_020", num_seqs=100):
    mpnn_seqs = []
    mpnn_seqs_af2 = []
    ...
    while len(mpnn_seqs) < num_seqs:
        ...
        # Process the sequence to remove 'X'
        seq = new_seq[0][-1][-1].strip().split("/")
        # Joining sequences with ';' and then removing 'X'
        newseq_sequence = ":".join(seq).replace("X", "")  # This line is modified
        newseq_sequence = "," + newseq_sequence
        if newseq_sequence not in mpnn_seqs:
            mpnn_seqs_af2.append([seq.replace("X", "") for seq in seq])  # Remove 'X' from each seq in list
...
def run_mpnn_on_diff_backbone(pdb_backbone, jsonfile, output_dir, mpnn_temp="0.1", mpnn_version="s_48_020", num_seqs=100):
    ...
    while len(mpnn_seqs) < num_seqs:
        ...
        seq = new_seq[0][-1][-1].strip().split("/")
        # Modify the following line to replace sequences of 'X' longer than 5 with ':'
        # and keep sequences of 'X' that are 5 or less unchanged
        newseq_sequence = [re.sub(r'X{6,}', ':', part) for part in seq]  # Process each part of seq
        # Joining sequences with ';'
        newseq_sequence = ":".join(newseq_sequence)  # ':' is used here to join, as per your original logic
        # Prepend ',' to the joined sequence
        newseq_sequence = "," + newseq_sequence
        if newseq_sequence not in mpnn_seqs:
            # For mpnn_seqs_af2, also apply the replacement, then remove 'X' entirely
            # Assuming you still want to remove all 'X' for mpnn_seqs_af2 as per your original code,
            # which might need adjustment based on your new requirements
            mpnn_seqs_af2.append([re.sub(r'X{6,}', ':', seq).replace("X", "") for seq in seq])
            mpnn_seqs.append(newseq_sequence)
    # Writing to file
    with open(os.path.join(output_dir, "sequences.csv"), 'w') as f:
        f.write("\n".join(mpnn_seqs))
...
if __name__ == "__main__":
    ...
    if args.pdb_backbone:
        ...
        for pdb in pdbs:
            ...
            if args.af2_preds_monomers:
                run_af2_on_mpnn_seqs_withmonomers(dist, pdb_backbone, work_list, score_func, num_preds, mpnn_dir)
            else:
                run_af2_on_mpnn_seqs_iter(dist, pdb_backbone, work_list, score_func, mpnn_dir)
