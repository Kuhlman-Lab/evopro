...
def run_genetic_alg_multistate(run_dir, af2_flags_file, score_func, startingseqs, poolsizes = [],
    ...
    while curr_iter <= num_iter:
        ...
        #SYDNEY: I think this is where to add normalizing code
        #scores is a list of tuples, where each tuple is (overall_score, split_scores, pdbs, results)
        ...
        for score, results, dsobj in zip(scores, results_packed, scoring_pool):
            ...
            print(scored_seqs[key_seq]["average"], len(scored_seqs[key_seq]["data"]), len(scored_seqs[key_seq]["data"][-1]))
        ...
        if write_pdbs:
            print("Writing pdbs...")
            for tup, j in zip(sorted_scored_pool, range(len(sorted_scored_pool))):
                print(tup)
                key_seq = tup[0]
                #key_seq = dsobj.get_sequence_string()
                num_preds = len(scored_seqs[key_seq]["data"])
                for k in range(num_preds):
                    pdbs = scored_seqs[key_seq]["data"][k]["pdb"]
                    #print(len(pdbs))
                    if type(pdbs) == str:
                        pdbs = [pdbs]
                    for pdb, chains in zip(pdbs, af2_preds):
                        with open(pdb_folder + "iter_" + str(curr_iter) + "_seq_" + str(j) + "_pred_" + str(k) +  "_model_1_chain"+str(chains)+".pdb", "w") as pdbf:
                                    pdbf.write(str(pdb))
