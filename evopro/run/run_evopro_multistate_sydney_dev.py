...
#modified repeat_af2 = False (previously was True but messed up normalization)
def run_genetic_alg_multistate(run_dir, af2_flags_file, score_func, startingseqs, poolsizes = [],
                               ...
                               repeat_af2=False, af2_preds=[], crossover_percent=0.2, vary_length=0, sid_weights=[0.8,0.1,0.1],
    ...
    print("this is what poolsizes is")
    print(poolsizes)
    ...
    while curr_iter <= num_iter:
        ...
        print("length of the starting pool is:")
        print(len(pool))

        if repeat_af2:
            scoring_pool = [p for p in pool if p.get_sequence_string() not in repeat_af2_seqs]
        else:
            # scoring_pool = [p for p in pool if p.get_sequence_string() not in scored_seqs] #problem was old sequences weren't being considered and so weren't being normalized
            if curr_iter == 1:
                scoring_pool = [p for p in pool if p.get_sequence_string() not in scored_seqs]
            else:
                scoring_pool = [p for p in pool if p.get_sequence_string() not in scored_seqs]
                for p in pool:
                    sequence = p.get_sequence_string()
                    if sequence in scored_seqs:
                        print("found sequence")
                        first_score = scored_seqs[sequence]["average"][0]
                        print(first_score)
                        #adding good scoring sequences into the sequence pool
                        if first_score != 1000:
                            print(f"adding {sequence} to pool")
                            scoring_pool.append(p)

                # all_seqs = list(scored_seqs.keys())
                # for seq in all_seqs:
                #     first_score = scored_seqs[seq]["average"][0]
                #     if first_score != 
                # print("ending score seqs format")
        print("length of the scoring pool is:")
        print(len(scoring_pool))
        ...
        if write_pdbs:
            print("Writing pdbs...")
            for i in range(len(scoring_pool)):
        ...
        print(f"the new pool size is {newpool_size}")
        if curr_iter < len(poolsizes):
            newpool_size = round(poolsizes[curr_iter]/2)
        else:
            pass
        newpool_seqs = []
        for sp in sorted_scored_pool[0:newpool_size]:
            newpool_seqs.append(sp[0])
        print("newpool", newpool_seqs)

        print(f"the length of the new pool is {len(newpool)}")
...
if __name__ == "__main__":
    ...
    run_genetic_alg_multistate(input_dir, input_dir + flagsfile, scorefunc, starting_seqs, poolsizes=pool_sizes, 
        ...
        repeat_af2=False, af2_preds = af2_preds, crossover_percent=args.crossover_percent, vary_length=args.vary_length, sid_weights=sid_weights,
