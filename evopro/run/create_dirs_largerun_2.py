...
def getFlagsParser() -> FileArgumentParser:
    ...
    parser.add_argument('--sequence_file',
                        default='chains.txt',
                        type=str,
                        help='Path to and name of sequence file.')
...
if __name__=="__main__":
    ...
    for perm, i in zip(seqs, range(len(seqs))):
        ...
        for j in range(args.num_replicates):
            ...
            os.system("python /proj/kuhl_lab/evopro/evopro/run/generate_json_dev.py @json.flags")
            os.chdir(main_dir)
