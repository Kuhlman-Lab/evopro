...
# modified import functions
from evopro.score_funcs.score_funcs import score_contacts, score_pae_confidence_pairs, score_pae_confidence_lists, score_plddt_confidence, get_rmsd, orientation_score, get_seq_indices, distance
from evopro.score_funcs.score_funcs import score_contacts_pae_weighted
import os
import subprocess
import shutil
import math
...
def score_binder_complex(results, dsobj, contacts, distance_cutoffs):
    ...
    if penalty_resids:
        penalty_contacts, penalty_contactscore = score_contacts_pae_weighted(results[0], pdb, penalty_resids, reslist2, dsobj=dsobj, first_only=False, dist=distance_cutoffs[2])
...
def score_binder_complex_with_rmsd(results, dsobj, contacts, distance_cutoffs, starting_pdb, binder_chain, contact_cap):
    from alphafold.common import protein
    print(results)
    print("")
    print(contacts)
    pdb = protein.to_pdb(results[0]['unrelaxed_protein'])
    chains, residues, resindices = get_coordinates_pdb(pdb)
    ...
    reslist1 = contacts[0]
    reslist2 = [x for x in residues.keys() if x.startswith("B")]
    contact_list, contactscore = score_contacts_pae_weighted(results[0], pdb, reslist1, reslist2, dsobj=dsobj, first_only=False, dist=distance_cutoffs[0])
    
    bonuses = 0
    bonus_resids = contacts[1]
    if bonus_resids:
        bonus_contacts, bonus_contactscore = score_contacts_pae_weighted(results[0], pdb, bonus_resids, reslist2, dsobj=dsobj, first_only=False, dist=distance_cutoffs[1])
        for contact in bonus_contacts:
            if contact[0][0:1] == 'A' and int(contact[0][1:]) in bonus_resids:
                bonuses += 1
                print("bonus found at: " + str(contact[0]))
...
def score_binder_rmsd_A(pdb1, pdb2, binder_chain="A", dsobj=None):
    chains1, residues1, resindices1 = get_coordinates_pdb(pdb1)
    chains2, residues2, resindices2 = get_coordinates_pdb(pdb2)
    reslist1 = [x for x in residues1.keys() if x.startswith(binder_chain)]
    reslist2 = [x for x in residues2.keys()]
