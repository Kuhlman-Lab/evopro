...
def score_binder_complex_3(results, dsobj, contacts):
    ...
    print(contacts)
    print(contacts[0])
    reslist1 = [x for x in contacts[0] if x.startswith("B")]
   # reslist2 = [x for x in residues.keys() if x.startswith("B")]
    reslist3 = [x for x in contacts[0] if x.startswith("C")] #added for proteinC
    reslist4 = [x for x in residues.keys() if x.startswith("A")] #added for proteinC
   ...
   # print("reslist2", reslist2)
    ...
    contacts_list, contactscore = score_contacts_pae_weighted(results, pdb, reslist1, reslist4, dsobj=None, first_only=False)
    contacts_list2, contactscore2 = score_contacts_pae_weighted(results, pdb, reslist3, reslist4, dsobj=None, first_only=False)
    score = -contactscore - contactscore2 # + nonpolar_penalty*5
...
def score_complex_confidence(results, dsobj):
    ...
    score = -confscore2*2 + plddt_potential #the bigger the difference betwenn plddt_cuttof and confscore the bigger the penalty
    print(score)
