...
from evopro.genetic_alg.diff_utils import create_new_seqs_diff_mpnn
...
def run_genetic_alg_binder(run_dir, af2_flags_file, score_func, startingseqs, poolsizes = [], binder_chain="B",
                               num_iter = 50, n_workers=1, mut_percents=None, contacts=None, contact_cap=100, distance_cutoffs=None,
                               mpnn_temps=None, mpnn_version="s_48_020", skip_mpnn=[], mpnn_iters=None, mpnn_chains=None,
                               repeat_af2=True, af2_preds=[], crossover_percent=0.2, vary_length=0, starting_pdb=None,
    ...
    print("Using number of contacts cap: ", contact_cap)
    lengths = []
    ...
    while curr_iter <= num_iter:
        ...
        #using protein mpnn to refill pool when specified
            ...
            if config_path:
                pool = create_new_seqs_diff_mpnn(pool, 
                                            scored_seqs, 
                                            poolsizes[curr_iter-1], 
                                            run_dir, 
                                            curr_iter, 
                                            ...
                                            diff_random_translate=diff_random_translate)
            else:
                print("WARNING: No config path provided for diffusion. Skipping diffusion and using just MPNN.")
        ...
        #scoring the predictions using the provided score function
        scores = []
        if not contacts:
    ...
    for key_seq, j in zip(newpool_seqs, range(len(newpool_seqs))):
        ...
        if conf_plot:
            ...
            for r, c in zip(result, af2_preds):
                ...
                pae_fig.savefig(output_dir + "seq_" + str(j) + "_final_"+str(c)+"_pae.png")

                # Plot pLDDTs.
                plddt_fig = plot_plddt(r['plddt'], Ls)
                plddt_fig.savefig(output_dir + "seq_" + str(j) + "_final_"+str(c)+"_plddt.png")
...
if __name__ == "__main__":
    ...
    if args.diffusion_config_path:
        config_path = args.diffusion_config_path
    else:
        config_path = None
    ...
    run_genetic_alg_binder(input_dir, input_dir + flagsfile, scorefunc, starting_seqs, poolsizes=pool_sizes, binder_chain=args.binder_chain,
        num_iter = args.num_iter, n_workers=args.num_gpus, mut_percents=mut_percents, contacts=contacts, contact_cap=args.contact_cap,
        mpnn_temps=mpnn_temps, mpnn_version=args.mpnn_version, skip_mpnn=mpnn_skips, mpnn_iters=mpnn_iters, mpnn_chains=mpnn_chains,
        repeat_af2=not args.no_repeat_af2, af2_preds = af2_preds, crossover_percent=args.crossover_percent, vary_length=args.vary_length, starting_pdb=args.path_to_starting,
        ...
        config_path=config_path, contigs=args.diffusion_contigs, provide_seq=args.provide_seq, diff_random_translate=args.diff_random_translate)