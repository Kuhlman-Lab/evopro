...
def score_overall(results, dsobj, contacts=None, distance_cutoffs=None, starting_pdb=None, binder_chain="B", contact_cap=100):
    ...
    print("Printing score terms: complex score, pLDDT score, rmsd score")
    print([x[0] for x in score])
    overall_score = sum([x[0] for x in score])
    return overall_score, score, pdbs, results

def score_overall2(results, dsobj, contacts=None, distance_cutoffs=None, starting_pdb=None, binder_chain="B", contact_cap=100):
    # modif overall score 
    from alphafold.common import protein
    print("Number of predictions being scored:", len(results))
    ...
    #print(score)
    score.append(score_binder_rmsd2(pdbs[0], pdbs[1], dsobj=dsobj, binder_chain=binder_chain))
    print("Printing score terms: complex score, pLDDT score, rmsd score")
    print([x[0] for x in score])
    print([x[1] for x in score])
    overall_score = sum([x[0] for x in score])
    score_details = [x[1] for x in score]
    # score needs to satisfy: overall_scores = overall_scores + [x[0] for x in score[1]]
    return overall_score, score, pdbs, results    
...
def score_binder_complex(results, dsobj, contacts, distance_cutoffs, binder_chain="B", contact_cap=100):
    ...
    print("80, Bonus resids:", bonus_resids)
    if bonus_resids:
        ...
        print("87, Bonus contacts:", bonus_contacts)
        print("88, Bonus score:", bonus_contactscore)
        for contact in bonus_contacts:
            ...
            if contact[0] or contact[1] in bonus_resids:
                bonuses += 1
                print("bonus found at: " + str(contact))
...
def score_binder_rmsd(pdb1, pdb2, binder_chain="B", dsobj=None):
    ...
    #print("136, Getting RMSD between:", reslist1, reslist2)
    rmsd_binder = get_rmsd(reslist1, pdb1, reslist2, pdb2, ca_only=True)
    print(f"138, RMSD between bound vs unbound: {rmsd_binder:.3f}")

    return (rmsd_binder*5, rmsd_binder)

def score_binder_rmsd2(pdb1, pdb2, binder_chain="B", dsobj=None):
    chains1, residues1, resindices1 = get_coordinates_pdb(pdb1)
    chains2, residues2, resindices2 = get_coordinates_pdb(pdb2)
    reslist1 = [x for x in residues1.keys() if x.startswith(binder_chain)]
    reslist2 = [x for x in residues2.keys()]
    #print("147, Getting RMSD between:", reslist1, reslist2)
    rmsd_binder = get_rmsd(reslist1, pdb1, reslist2, pdb2, ca_only=True)
    print(f"149, RMSD between bound vs unbound: {rmsd_binder:.3f}")

    return rmsd_binder, (rmsd_binder)    
...
def score_binder_rmsd_to_starting(pdb, path_to_starting, dsobj=None, binder_chain="B"):
    ...
    print("120, Getting RMSD between:", reslist1, reslist2)
    rmsd_to_starting = get_rmsd(reslist1, pdb_string_starting, reslist2, pdb, ca_only=True)
    print(f"123, RMSD to starting is: {rmsd_to_starting}")
    ...
    return rmsd_potential, (rmsd_to_starting, rmsd_potential)
...
def score_rmsd_to_starting(pdb, path_to_starting, dsobj=None):
    ...
    print("194, Getting RMSD between:", reslist1, reslist2)

    rmsd_to_starting = get_rmsd(reslist1, pdb_string_starting, reslist2, pdb, ca_only=True)
    print(f"197: RMSD to starting is: {rmsd_to_starting}")
